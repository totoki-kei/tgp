	...
	)
{
	...
	element.edge = input[0].emit.a * float4(1, 0, 0, 1) * AlphaBalance.xxxy;;
	output.Append(element);

	element.position = input[1].position;
	element.color = input[1].color;
	element.emit = input[1].emit;
	element.edge = input[1].emit.a * float4(0, 1, 0, 1) * AlphaBalance.xxxy;
	output.Append(element);

	element.position = input[2].position;
	element.color = input[2].color;
	element.emit = input[2].emit;
	element.edge = input[2].emit.a * float4(0, 0, 1, 1) * AlphaBalance.xxxy;
	output.Append(element);
}
